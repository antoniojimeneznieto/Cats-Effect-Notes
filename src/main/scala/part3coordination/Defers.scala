...
object Defers extends IOApp.Simple {
  ...
  def fileNotifierWithDeferred(): IO[Unit] = {
    ...
    for {
      ...
      fileTasksFib <- fileParts.parTraverse(part => downloadFilePart(part, contentRef, signal)).start //.map(part => downloadFilePart(part, contentRef, signal)).sequence.start
      _ <- notifierFib.join
      _ <- fileTasksFib.join
    } yield ()
  }
