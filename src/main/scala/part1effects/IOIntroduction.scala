...
object IOIntroduction {
  ...
  val anIO: IO[Int] = IO.pure {
    def function(): Int = 42 * 5
  ...
  val test = IO(StdIn.readLine()).flatMap(line1 => IO(StdIn.readLine()).flatMap(line2 => IO(println(line1 + line2))))
  ...
  val moreCombinedIO = (ourFirstIO, improvedMeaningOfLife, ourFirstIO).mapN((x, y, z) => x + y + z)
